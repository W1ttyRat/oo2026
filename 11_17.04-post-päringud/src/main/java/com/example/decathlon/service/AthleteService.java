...
public class AthleteService {
    ...
    public List<AthleteScoreDto> findAllAthletesWithScores() {
        ...
        return athletes.stream().map(athlete -> {
            long totalScore = athlete.getResults().stream().mapToInt(Result::getScore).sum();
            return new AthleteScoreDto(athlete.getId(), athlete.getName(), athlete.getCountry(), totalScore);
        }).toList();
    }

    public List<ResultViewDto> findResults(String discipline) {
        return resultRepository.findAllForView(discipline);
    }
}
